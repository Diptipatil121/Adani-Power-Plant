...
leaveRestForm.onsubmit = (e) => {
  e.preventDefault();
  if (firstName.value === "" || lastName.value === "" || employeeID.value === "" || initialRemainingDays.value === "") {
...
employeeID.addEventListener('input', async () => {
  ...
  const remainingDays = await getRemainingLeaveDays(employeeIDValue);
  initialRemainingDays.value = remainingDays;
});

async function getRemainingLeaveDays(employeeID) {
  const response = await fetch('leave_days.php', {
    method: 'POST',
